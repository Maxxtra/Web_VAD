...
def _get_patches(mspec, w, step):
    ...
    data = (data - np.mean(data, axis=1).reshape((len(data), 1))) / \
        np.std(data, axis=1).reshape((len(data), 1))
    lfill = [data[0, :].reshape(1, h*w)] * (w // (2 * step))
    rfill = [data[-1, :].reshape(1, h*w)] * \
        (w // (2 * step) - 1 + len(mspec) % 2)
    ...
    * outlabels: the labels associated the output of neural network models
    ...
    def __init__(self, batch_size):
        ...
        model_path = tf.keras.utils.get_file(
            self.model_fname, url + self.model_fname, cache_subdir='inaSpeechSegmenter')
        self.nn = tf.keras.models.load_model(model_path, compile=False)
    ...
    def __call__(self, mspec, lseg, difflen=0):
        ...
        *** output
        ...
        for lab, start, stop in lseg:
            ...
            pred = viterbi_decoding(np.log(r), diag_trans_exp(
                self.viterbi_arg, len(self.outlabels)))
            for lab2, start2, stop2 in _binidx2seglist(pred):
                ret.append(
                    (self.outlabels[int(lab2)], start2+start, stop2+start))
        return ret
...
#        self.graph = KB.get_session().graph # To prevent the issue of keras with tensorflow backend for async tasks
    ...
    def segment_feats(self, mspec, loge, difflen, start_sec, threshold_number):
        """
        do segmentation
        ...
        lseg = []
        for lab, start, stop in _binidx2seglist(_energy_activity(loge, threshold_number)[::2]):
            if lab == 0:
                lab = 'noEnergy'
            else:
                lab = 'energy'
    ...
    def __call__(self, medianame, tmpdir=None, start_sec=None, stop_sec=None, threshold_number=0.5):
        """
        ...
        * stop_sec (seconds): sound stream after stop_sec won't be processed
        ...
        mspec, loge, difflen = media2feats(
            medianame, tmpdir, start_sec, stop_sec, self.ffmpeg)
        if start_sec is None:
            start_sec = 0
        # do segmentation
        return self.segment_feats(mspec, loge, difflen, start_sec, threshold_number)
    ...
    def batch_process(self, linput, loutput, tmpdir=None, verbose=False, skipifexist=False, nbtry=1, trydelay=2., output_format='csv'):
        ...
        lmsg = []
        fg = featGenerator(linput.copy(), loutput.copy(),
                           tmpdir, self.ffmpeg, skipifexist, nbtry, trydelay)
        i = 0
        for feats, msg in fg:
            lmsg += msg
            i += len(msg)
...
def featGenerator(ilist, olist, tmpdir=None, ffmpeg='ffmpeg', skipifexist=False, nbtry=1, trydelay=2.):
    ...
    thread = ThreadReturning(target=medialist2feats, args=[
                             ilist, olist, tmpdir, ffmpeg, skipifexist, nbtry, trydelay])
    thread.start()
    while True:
        ret, msg = thread.join()
